...
export default function AtividadesPage() {
  ...
  useEffect(() => {
    ...
    const fetchAtividades = async () => {
      ...
      const unicas = data?.filter(
        (v, i, arr) =>
          arr.findIndex((x) => x.class_id === v.class_id && x.titulo === v.titulo) === i
      );
  ...
  // Abrir modal
  const handleAbrirModal = async (atividade: Atividade) => {
  ...
  const adicionarQAPair = () => {
    setQaPairs([...qaPairs, { pergunta: "", resposta: "" }]);
  };
  ...
  const gerarPDFAtual = () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text(`Atividade: ${atividadeSelecionada.titulo}`, 10, 20);
    doc.setFontSize(12);
    ...
    doc.save(`Atividade-${atividadeSelecionada.titulo}.pdf`);
  };
  ...
  const gerarPDFRespostasEnviadas = () => {
    ...
    doc.save(`Minhas-Respostas-${atividadeSelecionada.titulo}.pdf`);
  };

  const DefaultIcon = () => (
    <div className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center text-gray-400 text-lg font-bold">
      ?
    </div>
  );
          ...
          {atividades.length === 0 ? (
            ...
            atividades.map((a) => (
              <div
                ...
                className="bg-gray-900 border border-gray-800 p-5 rounded-xl hover:border-blue-500 transition cursor-pointer shadow-md hover:shadow-lg"
              >
                <h3 className="font-semibold text-lg text-white mb-1">{a.titulo}</h3>
                <p className="text-gray-400 text-sm">{a.materia} • {a.tipo}</p>
                <p className="text-gray-300 text-sm mt-2 line-clamp-3">{a.descricao}</p>
              </div>
            ))
          )}
        </div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            <div className="flex justify-between items-center border-b border-gray-800 p-4">
              <h2 className="text-xl font-semibold text-blue-300">
                💬 Respostas — {atividadeSelecionada.titulo}
              </h2>
              <button onClick={() => setModalAberto(false)} className="text-gray-400 hover:text-white text-lg">
                ✖
              </button>
            </div>

            {/* Lista de respostas rolável */}
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
              {respostas.length === 0 ? (
                ...
                respostas.map((r) => {
                  ...
                  const isProfessor = autor?.type === "teacher"; // COR BASEADA NO user.type
            ...
            <div className="border-t border-gray-800 bg-gray-900 p-4">
              ...
              {qaPairs.map((pair, i) => (
                <div key={i} className="flex flex-col gap-1 mb-2">
                  <input
                    ...
                    value={pair.pergunta}
                    onChange={(e) => atualizarQAPair(i, "pergunta", e.target.value)}
                    className="p-2 bg-gray-800 rounded-lg border border-gray-700 focus:ring-2 focus:ring-blue-500"
                  />
                  <textarea
                    placeholder={`Resposta ${i + 1}`}
                    value={pair.resposta}
                    onChange={(e) => atualizarQAPair(i, "resposta", e.target.value)}
                    rows={2}
                    className="p-2 bg-gray-800 rounded-lg border border-gray-700 focus:ring-2 focus:ring-blue-500 resize-none"
                  />
                </div>
              ))}
              ...
              <div className="flex gap-2 mb-2">
                <textarea
                  placeholder="Digite uma mensagem normal..."
                  value={mensagem}
                  onChange={(e) => setMensagem(e.target.value)}
                  rows={2}
                  className="flex-1 p-2 bg-gray-800 rounded-lg border border-gray-700 focus:ring-2 focus:ring-blue-500 resize-none"
                />
              ...
              <button onClick={gerarPDFAtual} className="w-full py-2 bg-purple-600 hover:bg-purple-700 rounded-lg font-semibold mb-2">📄 Exportar PDF (atual)</button>
